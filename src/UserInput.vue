<template>
  <div>
    <Suggestions v-if="getSuggenstionsButtns" :suggestions="suggestions" v-on:sendSuggestion="_submitSuggestion"
                 :colors="colors"/>

    <div v-if="!getSuggenstionsButtns" style="display: flex; justify-content: center">
      <img style="height: 60px;width: 60px; margin-bottom: 15px; "
           src="https://forge.codesys.com/forge/support/_discuss/thread/6f37666443/dbe4/attachment/ajax-loader.gif"
           alt="">
    </div>
    ...
    <form v-if=" !getSuggenstionsButtns || getCurrentKey !== '' || !getToken  " class="sc-user-input"
          :class="{active: inputActive}" :style="{background: colors.userInput.bg}">
      <div
        role="button"
        tabIndex="0"
      ...
      <div class="sc-user-input--buttons">
        ...
        <div class="sc-user-input--button">
          <user-input-button @click.native.prevent="_editText" v-if="isEditing" :color="colors.userInput.text"
                             tooltip="Edit">
            <icon-ok/>
          </user-input-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapMutations(
        {
          suggenstionsButtns: 'chat/suggenstionsButtns',
        },
      ),
      ...mapActions(
        {
          uploadFileData: 'chat/uploadFileData',
        },
      ),
      cancelFile() {
        this.file = null
    ...
    computed: {
      ...mapGetters(
        {
          getSuggenstionsButtns: 'chat/getSuggenstionsButtns',
